...
import '../widgets/pagination/core_pagination_page_button.dart';
import '../widgets/pagination/core_pagination_ellipsis.dart';

/// Core utility class for pagination calculations and widget building
/// Centralized for reuse across the entire application
class CorePaginationUtils {
  ...
  static List<Widget> buildPageNumbersList({
    ...
    if (totalPages <= 1) {
      pages.add(CorePaginationPageButton(
        page: 1,
    ...
    if (totalPages > 2) {
      ...
      if (start > 2) {
        pages.add(const CorePaginationEllipsis());
      }
