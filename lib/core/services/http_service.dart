...
class HttpService implements IHttp {
  ...
  Future<ServerRequest> requestHttp({
    ...
    try {
      ...
      endpoint = kDebugMode
          ? '${ApiEndpoints.devUrl}${ApiEndpoints.base}$endpoint'
          : '${ApiEndpoints.base}$endpoint';

      log("[http_service.dart -> requestHttp()] Sending '$apiMethod' Request to | endpoint: $endpoint'...");

      Uri uri = Uri.parse(endpoint);

      // Add query parameters to the request if provided
      if (params != null) {
        log("[http_service.dart -> requestHttp()] [$endpoint] Adding query parameters: $params...");
      ...
      if (fileFieldName != null && ((kIsWeb && fileBytes != null) || (!kIsWeb && filePath != null))) {
        log("[http_service.dart -> requestHttp()] [$endpoint] Preparing multipart file upload...");
        ...
        try {
          ...
          if (kIsWeb) {
            ...
            log("[http_service.dart -> requestHttp()] [$endpoint] Added file from bytes for web: ${fileName ?? 'file.csv'}");
          } else {
            // For mobile/desktop: use file path
            if (filePath == null) {
              throw Exception("File path is required for mobile/desktop file upload");
            }
            multipartFile = await http.MultipartFile.fromPath(
              fileFieldName,
              filePath,
            );
            log("[http_service.dart -> requestHttp()] [$endpoint] Added file from path: $filePath");
          }
          
          request.files.add(multipartFile);
          log("[http_service.dart -> requestHttp()] [$endpoint] File added successfully with field name: $fileFieldName");
          
        } catch (e) {
          log("[http_service.dart -> requestHttp()] [$endpoint] Error adding file: $e");
        ...
        if (body != null) {
          ...
          log("[http_service.dart -> requestHttp()] [$endpoint] Added form fields: $body");
        }
        ...
        log("[http_service.dart -> requestHttp()] [$endpoint] Sending multipart request...");
        ...
        if (body != null) {
          log("[http_service.dart -> requestHttp()] [$endpoint] Appending Body: '$body'...");
      ...
      log("[http_service.dart -> requestHttp()] [$endpoint] Status Code: ${response.statusCode}");
      ...
      switch (response.statusCode) {
        ...
        case 401:
          log("[http_service.dart -> requestHttp()] Error: ${serverResponse.response}. Showing SnackBar...");
      ...
      log("[http_service.dart -> requestHttp()] A Format Exception error has occurred. Please review app logic.");
      if (context.mounted) {
        SnackbarServiceHelper.showError(
          "$endpoint\nFormat Exception:",
          position: SnackPosition.BOTTOM,
